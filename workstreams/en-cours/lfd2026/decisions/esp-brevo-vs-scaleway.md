# Comparatif ESP — Brevo vs Scaleway (vs autres) pour LFD 2026
...
| Scénario | Choix reco |
|---|---|
| **Priorité souveraineté MEAE (le cas ici)** | 🥇 **Scaleway TEM** — *sous réserve du test de délivrabilité* → sinon 🥈 **Brevo** |
...
- [ ] ESP retenu : **Scaleway TEM / Brevo / autre** → _____
- [ ] Test d'inbox placement réalisé (Gmail/Outlook/Orange/Free) → résultat _____
- [ ] IP dédiée ou mutualisée → _____
- [ ] **Rate-limit (emails/s) confirmé par l'ESP** → _____
- [ ] **Quota journalier confirmé ≥ ~15 000/jour** (pour ~20k sur 2 j) → _____
- [ ] **Test rafale réalisé** (500-1000 emails en staging, sans throttling) → _____
- [ ] Worker BullMQ plafonné au rate ESP → _____
- [ ] SPF/DKIM/DMARC configurés (date) → _____
- [ ] Warm-up lancé (date début) → _____
- [ ] Quota d'envoi confirmé ≥ pic attendu → _____
