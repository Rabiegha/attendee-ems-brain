...
>
> **Dernière mise à jour :** 2026-07-13 soir (**0-CI : pipeline validé bout en bout** — prérequis ops ✅, CI verte 18/18, CD staging back ✅ en conditions réelles, workflows sur `main`, CD prod ⏸ en attente · **C1 débloquée : accès OVH obtenus, warm-up démarré en starter** — 🔴 purchase plan Mailgun bloqué (bug billing), retry mercredi 15/07 · nouveau chantier T · H phases 0-1 back faites)
...
```
Avancement global périmètre event : ▓▓▓▓▓░░░░░░░░░░░░░░░ ~25 %
...
> hors C1 calendaire / BIL non estimé / F-G reportés). Passage 22 → **~25 %** le 13/07 au soir :
> 0-CI n'est plus « code poussé non livré » mais **validé en conditions réelles** (CI verte,
> CD staging back rodé sur le VPS) et C1 est **débloquée** (warm-up lancé). Reste retenu sous le
> brut car CD prod pas encore rodé et T (P0) pas commencé.
...
| **0-CI**  | CI/CD minimal (build+test PR + deploy staging)                                                                                                                                                                                 | Rabie      | ~2 j               | ~1,5 j                  | **90 %** `▓▓▓▓▓▓▓▓▓░`  | 🟢 **Pipeline validé bout en bout (13/07 soir)** : prérequis ops ✅ (clé SSH, secrets, PAT GHCR) · CI verte 18/18 e2e (3 fixes : redis, seed e2e, throttler) · **CD staging back rodé en conditions réelles** (image GHCR → VPS, health ok) · workflows sur `main` · 2 incidents nginx corrigés — reste : CD prod back/front (⏸ fenêtre 22h-06h), rodage staging front, branch protection | S29             |
