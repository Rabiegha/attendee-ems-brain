...
>
> **Dernière mise à jour :** 2026-07-24 (**chantier H fermé côté code et en PR**) :
> H-D8 est maintenant appliqué dans la frontière transactionnelle commune : maximum deux sessions
> `confirmed` par jour `Europe/Paris`, waitlist hors quota et protection sous concurrence.
> **24/24 E2E H sont verts**, la V1 front est mode-aware, le build back et le typecheck/build front
> sont verts. PR ouvertes vers `staging` : back
> [#68](https://github.com/Rabiegha/attendee-ems-back/pull/68) et front
> [#27](https://github.com/Rabiegha/attendee-ems-front/pull/27). H reste à 95 % tant que merge/CI,
> migration + smoke prod et contre-recette appareils M/K ne sont pas fermés.
...
| **H** | Inscriptions par session (lien public + capacité/waitlist + front + limite CDC/jour) | Corentin | 🔴 P0 — cœur métier LFD : choix de sessions, limites et capacité | **~6,5–10,5 j** | ~3,5–4 j | **95 %** `▓▓▓▓▓▓▓▓▓▓` | 🟢 **Code V1 terminé, 24/24 E2E H verts.** Socle sessions, capacité/waitlist, check-in distinct, compteurs atomiques, contrat 409 `ALREADY_SESSION_SCANNED` et concurrence dernière place livrés. **H-D8** : maximum deux choix `confirmed` par jour `Europe/Paris`, waitlist hors quota, troisième choix refusé sans toucher la jauge et trois demandes concurrentes sérialisées. Front V1 mode-aware : walk-in = présences uniquement ; avec inscription = inscrits/capacité/waitlist/contrôles. PR back #68 + front #27 ouvertes vers `staging`. Restent comme gates : merge/CI, migration + smoke prod et contre-recette appareils M/K ; l'audit détaillé du re-scan appartient à O. Aucun double comptage M/H/L9.1/J. | S31 — gates livraison |
| **J** | Capacité live forte charge + [J-ENTREES](./J-capacite-live/README.md#j-entrees--tableau-de-bord-des-entrées-lfd) | binôme | 🔴 P0 — couvre les 3 000 requêtes, les entrées live et la surcapacité | **~5–9 j** | inclus H + L9b + ~1,5 j | **80 %** `▓▓▓▓▓▓▓▓░░` | 🟢 **Implémentation J-ENTREES mergée et déployée staging** (back #66/#67, front #26, CI/CD verts) : compteur atomique `unique_entered_count`, snapshot event `GET /events/:eventId/sessions/stats`, export `GET /events/:eventId/sessions/:id/export-presence`, WebSocket `session:checkin-stats`, dashboard « Présences » et CSV. Le harnais k6 check-in (smoke, dashboard poll, baseline, vérification compteurs) est mergé. **Reste volontairement reporté :** exécution check-in/combinée, preuve 3 000 simultanées et décision portier Redis selon mesures. | S30–S31 |
