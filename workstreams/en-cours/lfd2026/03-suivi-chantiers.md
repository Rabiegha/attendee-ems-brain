...
| **H**     | Inscriptions par session (lien public + capacité/waitlist + front)                                                                                                                                                             | à répartir | ~5,5–8,5 j         | ~1 j                    | **45 %** `▓▓▓▓▓░░░░░`  | 🟢 Phases 0-1 back FAITES (schéma+migration, routes publiques session, token admin, anti-doublon assoupli, fix `session_choice_ids`) + back-office câblé (credentials supprimés) · reste front ph.2 + tests + `registration_opens_at` (en cours) · non déployé (PR en fin de chantier)                                                                                                                                                           | S29→S31         |
