...
- Une règle métier validée existe : un User ne peut avoir qu'**une seule Organization Free active créée par lui** (voir [../steering/DECISIONS.md](../steering/DECISIONS.md#free-plan-rule)).

## 3. What is already decided

Voir [../steering/DECISIONS.md](../steering/DECISIONS.md) :
...
- Comportement quand un User est désinvité / quitte une org (lien avec [docs/bugs/user-deletion-multitenant-membership.md](../bugs/user-deletion-multitenant-membership.md)).
- Limites par plan applicables dès l'onboarding (compteur events, registrations, prints, users).

## 5. Why not now

Le focus actuel est **Async Architecture** (voir [../steering/NOW.md](../steering/NOW.md)). Toucher au parcours onboarding sans avoir stabilisé les fondations async + bugs sécurité ouverts (CORS, user deletion) introduirait du risque inutile.
...
- Après décision humaine sur les questions BLOCKING listées dans [../async-architecture/QandA/12-open-questions-for-human.md](../workstreams/async-architecture/QandA/12-open-questions-for-human.md).
...
- Lier les décisions actées dans [../steering/DECISIONS.md](../steering/DECISIONS.md).
