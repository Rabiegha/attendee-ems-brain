...
| T-14 | **Moteur PDF badge → Option D (Gotenberg/Cloud Run) actée, MAIS 2 garde-fous avant prod.** (1) Le **~1,5 s/badge** est une **hypothèse non mesurée** — le load test du 25/06 ne générait pas de badges. (2) Le rendu Cloud Run **n'est pas validé** vs Puppeteer. | back/infra | 🟠 Haute | (1) Ajouter un **scénario badge** au **load test S4** pour mesurer le vrai temps + l'impact sur la latence inscription. (2) **Comparaison visuelle pixel-à-pixel** obligatoire avant bascule prod ; garder **Puppeteer local en fallback** jusque-là. Réf : `workstreams/en-cours/lfd2026/decisions/lib-pdf-badge.md` §5-6. |
